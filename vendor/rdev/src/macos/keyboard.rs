...
impl Keyboard {
    ...
    pub(crate) unsafe fn create_string_for_key(
        ...
        // EmulStick patch: skip the Text Services Manager lookup.
        //
        // `string_from_code` calls TISCopyCurrentKeyboardInputSource /
        // UCKeyTranslate, which assert they run on the main thread. Because we
        // run `grab` on a dedicated background thread (Tauri owns the main
        // thread), that assertion fires `dispatch_assert_queue` → SIGTRAP on
        // every key event. We never read `Event::name`, so returning `None`
        // here is harmless and keeps the grab thread alive. See PATCH.md.
        None
    }
