...
> The EmulStick dongle is a commercial product — see **[emulstick.com](https://emulstick.com)** for the hardware and where to buy. This repository is an independent third-party desktop operator client for compatible hardware.
...
- **Backend** (`src-tauri/src/`):
  - `protocol/` — HID report encoders (keyboard 8 B / mouse 6 B), the `rdev::Key → HID usage` keymap, and the BLE UUIDs used by this implementation. Hardware-independent and unit-tested byte-for-byte against the worked examples in [`docs/protocol.md`](docs/protocol.md).
...
See [`docs/plan.md`](docs/plan.md) for the full engineering design, and [`docs/protocol.md`](docs/protocol.md) / [`docs/ble-protocol.md`](docs/ble-protocol.md) for implementation-focused protocol notes. For the vendor's authoritative protocol documentation, see the official [EmulStick BLE protocol specification (v0.93)](https://www.emulstick.com/files/emulstick_ble_v0.93.pdf).
...
Copyright for the original project material is retained by the project author. No general software license is currently granted for the original project code. See [`COPYRIGHT`](COPYRIGHT) for the project notice and [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) for third-party components and their licenses.
